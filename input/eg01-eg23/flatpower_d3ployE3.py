...
The user can choose a demand equation (demand_eq), a buffer, and the number of 
look-ahead steps calculated by the prediction methods by running:
python flatpower_d3ployE3.py <buff_size> <for_steps>

for ex: to define a buffer of 2000 MW and 2 forward steps
python flatpower_d3ployE3.py 2000 2

The installed capacity feature is not used.
The buffer starts playing its role one time step before the transition.
"""
...
for calc_method in calc_methods:
    ...
    """%(calc_method, demand_eq, for_steps, buff_size, calc_method, for_steps)

for calc_method in calc_methods:

    input_file = 'eg01-eg23-flatpower-d3ploy-bufferB' + buff_size + '-' + for_steps + '-' + calc_method + '.xml'
    output_file = 'eg01-eg23-flatpower-d3ploy-bufferB' + buff_size + '-' + for_steps + '-' + calc_method + '.sqlite'
